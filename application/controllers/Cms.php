...
class Cms extends CI_Controller {
    ...
    public function menu($type = 'side menu')
    {
        ...
        $template_data['pengaturan']=$this->Pengaturan_model->get_by_id_1();           
    ...
    public function get_nestable_menu($menus, $parent_id = 0)
    {
        ...
        foreach ($menus as $menu) {
            if ($parent_id == $menu['parent_id']) {
                ...
                <div class="dd-handle '.$this->config->item('header').' "><i class="fa fa-ellipsis-v"></i> <i class="fa fa-ellipsis-v"></i></div><p>'.$menu['label'].'
                <span class="dd-action">
                  <a href="'.site_url('menu/update/'.$menu['id_menu']).'" title="Edit" class="delete-confirm btn btn-xs btn-warning btn-flat"><i class="fa fa-edit"></i></a>
                  <a href="#" title="Delete" class="delete-confirm btn btn-xs btn-danger btn-flat" onclick="return confirmdelete(\'cms/crud_menu/'.$type.'/delete/'.$menu['id_menu'].'\')")><i class="fa fa-trash"></i></a>
              </span></p>';
